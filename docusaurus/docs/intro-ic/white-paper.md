...
&emsp;&emsp;回想一下，DKG协议（详见[章节3.5](#3.5 基础DKG协议)）要求，对于每个签名密钥，有一个dealing的集合，而每个节点副本可以根据这组dealing，非交互式得获取它的签名密钥片段。

&emsp;&emsp;再回想一下，除了别的之外，NNS还维护着决定子网成员的**注册表**（详见[章节1.5](#1.5 权限模型)）。注册表（以及子网成员）会随时间改变。因此，子网必须在不同时间出于不同目的，对使用的**注册表版本**达共识。这一信息也存储在摘要块中。
...
![图5：快速转发](img/%E5%9B%BE5%EF%BC%9A%E5%BF%AB%E9%80%9F%E8%BD%AC%E5%8F%91.png)
...
**协议升级：**CUP也可用于协议升级。协议升级由NNS发起（详见[章节1.5](#1.5 权限模型)）。不考虑所有细节，基本细节如下：
